...
/**
 * Implements the internal processing functionality for the new patch form.
 */

if(!function_exists('wordpatch_newpatch_process_internal')) {
    /**
     * Internal processing for the new patch form. Responsible for validation and persistence.
     *
     * @param $wpenv_vars
     * @param $current
     * @param $job_id
     * @return array
     */
    function wordpatch_newpatch_process_internal($wpenv_vars, $current, $job_id) {
        ...
        if($current['patch_type'] === wordpatch_patch_type_file() && (!$current['patch_file_uploaded'] || !wordpatch_did_file_upload_succeed())) {
        ...
        // Create an ID for the new patch.
        $patch_id = wordpatch_unique_id();
        ...
        $patch_location = wordpatch_file_upload_generate_location();

        // Create a location for the old file (simple patch)
        $patch_location_old = wordpatch_file_upload_generate_location();

        // Create a location for the new file (simple patch)
        $patch_location_new = wordpatch_file_upload_generate_location();
        ...
        // Try to persist the database record.
        $db_error = wordpatch_newpatch_persist_db($wpenv_vars, $current, $job_id, $patch_id, $patch_location,
            $patch_location_old, $patch_location_new);
        ...
        wordpatch_redirect(wordpatch_editjob_uri($wpenv_vars, $job_id, null, null, $patch_id));
        exit();
    }
}